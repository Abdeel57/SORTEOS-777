...
export class DatabaseSetupService {
  ...
  async ensurePhoneNumbersColumns() {
    try {
      const phoneNumbersExists = await this.prisma.$queryRaw<Array<{ column_name: string }>>`
        SELECT column_name 
        FROM information_schema.columns 
        WHERE table_name = 'settings' AND column_name = 'phoneNumbers'
      `;
      if (phoneNumbersExists.length === 0) {
        await this.prisma.$executeRawUnsafe(`ALTER TABLE "settings" ADD COLUMN IF NOT EXISTS "phoneNumbers" JSONB;`);
        this.logger.log('✅ Added phoneNumbers column to settings table');
      }

      const phoneNextIndexExists = await this.prisma.$queryRaw<Array<{ column_name: string }>>`
        SELECT column_name 
        FROM information_schema.columns 
        WHERE table_name = 'settings' AND column_name = 'phoneNextIndex'
      `;
      if (phoneNextIndexExists.length === 0) {
        await this.prisma.$executeRawUnsafe(`ALTER TABLE "settings" ADD COLUMN IF NOT EXISTS "phoneNextIndex" INTEGER NOT NULL DEFAULT 0;`);
        this.logger.log('✅ Added phoneNextIndex column to settings table');
      }
    } catch (error) {
      this.logger.warn('⚠️ Error checking/adding phone numbers columns to settings table:', error);
    }
  }
}
